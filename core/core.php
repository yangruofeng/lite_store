...
spl_autoload_register(array('classLoader','load_file'));
...
function getFinanceUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style, FINANCE_SITE_URL);
}
...
class result
{
    ...
    function __construct($_sts=false, $msg = "", $data = null, $_code = 0, $inner_result = null)
    {
        $this->STS = $_sts;
        if (is_null($_code)) {
 ...
 * @return string 字符串类型的返回结果
 */
function decryptY($txt, $key = '', $ttl = 0)
{
    if (empty($txt)) return $txt;
    if (empty($key)) $key = md5(MD5_KEY);
...
function checkPoneNumber($phone_number)
{
    // /^(13[0-9]|14[0-9]|15[0-9]|166|17[0-9]|18[0-9]|19[8|9])\d{8}$/
    return preg_match('/^(13[0-9]|14[0-9]|15[0-9]|166|17[0-9]|18[0-9]|19[8|9])\d{8}$/', $phone_number);
}
...
function getUidByObjGUID($obj_guid, $obj_type = objGuidTypeEnum::CLIENT_MEMBER)
{
    //去掉最前面的数字
    $str = substr($obj_guid, strlen($obj_type));
    return intval($str);
}
...
function roundX2($i)
{
    ...
    if ($i_start == 3) {
        ...
        $str_target = str_pad("", $i_start - 3, "4", STR_PAD_LEFT);
        $str_target = "0.00" . $str_target . "5";
    }
    $new_id = number_format($i - 0.005, 2);
    $i_new = $new_id;
    $new_id = str_replace(",", "", $new_id);
    $new_id = str_pad($new_id, $i_len, "0", STR_PAD_RIGHT);
    $i_diff = bcsub($i, $new_id, $i_len);

    $i_comp = bcsub($i_diff, $str_target, strlen($str_target) - 2);
    if ($i_comp >= 0) {
        $ret = round($i_new, 2) + 0.01;
    } else {
        $ret = $i_new;
    }
    return $ret;
 ...
 * @return mixed
 */
function XMLParse($xml_data, $is_array = true)
{
...
/*
     * 金额按币种最小金额格式化
     */
function roundAmountByCurrency($amount,$currency)
{
    switch( $currency ){
        case 'USD':
