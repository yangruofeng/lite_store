...
class ormYo{
    ...
    public static function wlog($log_type=null,$content=''){
    ...
    public static function Reader($dsn=null){
        $c=self::Conn($dsn);
...
class ormDataColumn implements IormObj{
    ...
    function __construct($_name=null,$_caption=null ){
        $this->name=$_name;
        if(!$_caption){
            $_caption=$this->name;
        }
...
class ormDataRow extends ArrayObject implements IormObj{
    ...
    public function getChild($chdName){
        ...
        if(!$this->table->children) return null;
        $chd = null;
    ...
    public function resetChildren($crows=array()){
...
class ormChild implements IormObj{
    ...
    public $table;
    public $parentTable;
    public function __construct($_pcols=null,$_ccols=null,$schema=null,$_name=null,$dsn=null){
...
class ormDataTable implements IormObj{
    ...
    public function fill($condition=null,$page=null,$pageSize=null,$sql_spec='',$group_by='',$having='',$order_by=''){
    ...
    private function callTrigger($sts,ormDataRow $clone,ormDataRow $current){
        ...
        if(!$tg) return;
        switch($sts){
            case IormTrigger::BEFORE_INSERT:
                $tg->before_insert($clone,$current);
                break;
            case IormTrigger::BEFORE_UPDATE:
                $tg->before_update($clone,$current);
                break;
            case IormTrigger::BEFORE_DELETE:
                $tg->before_delete($clone,$current);
                break;
            case IormTrigger::AFTER_INSERT:
                $tg->after_insert($clone,$current);
                break;
            case IormTrigger::AFTER_UPDATE:
                $tg->after_update($clone,$current);
                break;
            case IormTrigger::AFTER_DELETE:
...
trait ormTrait{
    ...
    public  function _formatCondition($condition=null) {
    ...
    private static function _formatStr($s=null, $prefix='', $endfix='') {
        if (!$endfix)
            $endfix = "";
        if (!$prefix)
            $prefix = "";
...
class ormResult implements IormObj{
    ...
    function __construct($_sts=false,$_error_no=0,$_msg='',$_src_col=null,$_auto_id=null,$_count=null,$_result=null){
        $this->STS=$_sts;
...
class ormSqlCommand{
    ...
    public function run(){
        if(!$this->commandText) return new ormResult(false);
