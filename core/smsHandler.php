...
class smsHandler
{
    ...
    public function sendLoanRepaymentAlert($country_code,$phone_number,$payment_date,$payment_detail)
    {
        //检查数据
        ...
        //构造短信模版
        $content = "【Samrithisak】សូមជម្រាបអតិថិជន ថ្ងៃដែលលោកអ្នកត្រូវសងប្រាក់គឺនៅថ្ងៃទី $payment_date ចំនួនទឹកប្រាក់សរុប $payment_detail ប្រសិនបើសមតល្យមិនគ្រប់គ្រាន់ សូមបញ្ចូលទឹកប្រាក់បន្ថែម ឬទៅការិយាល័យសំរិទ្ធីស័ក៕ សូមអរគុណ";


        //第一步插入数据库
        $insert = $this->insertSmsTask(smsTaskType::LOAN_REPAYMENT_ALERT, $contact_phone, $content);
        ...
        if (C("sms_api") == 'yunpian') {
            $api = new sms_api_yunpian();
            $st = $api->sendSmsToYP($contact_phone, $content);
            $task->handle_result = json_encode($st, true);
            ...
            if (isset($st['code']) && $st['code'] == 0) {
                ...
                $task->update_time = Now();
                $task->update();
                return new result(false, 'Send sms fail',$task);
            }
            ...
            $api = new sms_api_tencent();
            $st = $api->sendLoanRepaymentAlert(array(
                'mobile' => $phone_number,
                'nationcode' => $country_code
            ), $payment_date,$payment_detail);
            $task->handle_result = json_encode($st, true);
