...
require_once $dir."/WxPay.Config.Interface.php";
...
class WxPayApi
{
	...
	public static function notify($config, $callback, &$msg)
	{
		...
		debug('WX_CALL_BACK_XML.txt',$xml);
		if (empty($xml)) {
			# 如果没有数据，直接返回失败
			return false;
		}
		...
		try {
			...
			debug('wx_notify.txt','Init fail:'.$e->getMessage());
			$msg = $e->errorMessage();
			return false;
		}
	...
	{		
		...
		if($data){
			...
			$error = curl_errno($ch);print_r($error);die;
			curl_close($ch);
