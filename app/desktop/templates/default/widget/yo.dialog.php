    ...
    <div class="weui-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer default-footer">
                <a class="btn btn-got-it" style="font-size: 15px;font-weight: bold;color: #09BB07;text-decoration: none" onclick="javascript:$('#iosDialog').hide();">知道了</a>
                <button type="button" class="btn btn-primary btn-confirm">确认</button>
                <button type="button" class="btn btn-default btn-cancel" data-dismiss="modal" onclick="javascript:$('#iosDialog').hide();">取消</button>
            </div>
...
<script>
    ...
    yo.confirm=function(_title,_desc,_fn){
        ...
        if(_title){
            ...
            $("#iosDialog").find(".modal-title").text('提示');
        }
        ...
        $("#iosDialog").find(".btn-cancel").show().unbind().on("click",function(){
            $("#iosDialog").hide();
            if($.isFunction(_fn)){
                _fn(false);
            }
        });
        $("#iosDialog").find(".btn-confirm").show().unbind().on("click",function(){
            $("#iosDialog").hide();
            if($.isFunction(_fn)){
                _fn(true);
            }
    ...
    yo.dialog.show=function(_args){
        ...
        if(_args.title){
            ...
            _dialog.find(".modal-title").text('提示');
        }
        if(_args.body_css){
    ...
    yo.dialog.prompt=function(_title,_desc,_fn,_default_value){
        ...
        yo.dialog.show({
            ...
            buttons:[{text:"Submit",cls:"btn btn-primary",handler:function(){
                ...
                yo.dialog.close();
                if(_fn){
                    //_fn(null); // 取消就不调用这个方法了
                }
            }}]
        });
        yo.dialog.body().find(".txt_prompt").focus();
    };
    yo.dialog.password=function(_title,_fn){
        if(!_default_value) _default_value="";
        var _frm='<div><input type="password"  class="txt_prompt_password form-control"  placeholder="input password" style="width: 100%" /></div>';
        _frm=$(_frm);
        yo.dialog.show({
            content:_frm,
            title:_title,
            buttons:[{text:"Submit",cls:"btn btn-primary",handler:function(){
                var _value=yo.dialog.body().find(".txt_prompt_password").val();
                yo.dialog.close();
                if(_fn){
                    _fn(_value);
                }
        ...
        yo.dialog.body().find(".txt_prompt_password").focus();
    };
