...
//这里得到的rowData都是一行的数据，得到数据后自行处理，我们这里只打出来看看效果
    ...
    public function testPageOp()
    {
        $r = new  ormReader();
        $sql = "select * from common_sms ";
        $pageNumber = 1;
        $pageSize = 0;
        $page = $r->getPage($sql,$pageNumber,$pageSize);
        unset($page->rows);
        print_r($page);
    }


    public function testImageCompressOp()
    {
        Tpl::showPage('image.compress.page');
    }
